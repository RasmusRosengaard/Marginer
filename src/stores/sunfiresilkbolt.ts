...
import { SUNFIRE_SILK_BOLT_RECIPE } from '@/config/recipes'
import { SUNFIRE_SILK_BOLT_ITEM_IDS } from '@/config/items'
import type { ItemQuality, WowItem } from '@/config/items'

const RESOURCEFULNESS_SAVE_RATIO = 0.30

export const useSunfireSilkBoltStore = defineStore('sunfiresilkbolt', () => {
  ...
  const recipe = SUNFIRE_SILK_BOLT_RECIPE
  ...
  async function loadMarketData() {
    ...
    try {
      const [priceResult, mediaData] = await Promise.all([
        fetchPrices(SUNFIRE_SILK_BOLT_ITEM_IDS),
        fetchMedia(SUNFIRE_SILK_BOLT_ITEM_IDS)
      ])
