...
const profession = computed(() => PROFESSIONS.find(p => p.slug === slug.value))
</script>

<template>
  <div v-if="profession" class="profession-page">
    <div
      v-motion
    ...
    >
      ...
      <div>
        ...
        <p class="subtitle">
          {{ profession.crafts.length > 0 ? 'Select a recipe to calculate its profit margin' : 'No recipes added yet — check back soon.' }}
        </p>
      </div>
    </div>

    <ul v-if="profession.crafts.length > 0" class="craft-list">
      <li
      ...
      >
        <RouterLink :to="`/${profession.slug}/${craft.slug}`" class="craft-link">
...
.profession-icon {
  ...
  height: 52px;
  border-radius: 8px;
  border: 1px solid rgba(var(--a), 0.50);
  box-shadow: 0 0 14px rgba(var(--a-mid), 0.30);
}
...
.craft-link:hover {
  ...
  box-shadow: 0 0 18px rgba(var(--a-mid), 0.28);
}
